...
if (isset($_POST["Delete"])) {
        $db->query("DELETE FROM medicines WHERE Title = '{$fa}'");
        header('location: admin.php');
}

$Diag=$db->query("SELECT * FROM medicines WHERE Title = '{$fa}'");

if (isset($_POST["Back"])) {
        $db->query("UPDATE medicines SET Title = '{$_POST['DD']}' WHERE Id = '{$Diag[0]['Id']}'");
        $db->query("UPDATE medicines SET Dosage = '{$_POST['Dosage']}' WHERE Id = '{$Diag[0]['Id']}'");
        $db->query("UPDATE medicines SET CourseDuration = '{$_POST['text']}' WHERE Id = '{$Diag[0]['Id']}'");
        header('location: admin.php');  
				...
				<form method="POST">
					<div class="col-md-12">		
							...
							<div class="col-md-4 text-center">
								...
								<div class="col-md-2 text-center">
								<p class="myCardd downPriem"><strong>Доза:</strong> </p>							
							</div>
							<div class="col-md-4 text-center">
							<input type="text"  name="Dosage" placeholder="<?=$Diag[0]['Dosage']; ?>">						
							</div>
							
							<div class="col-md-2 text-center">
								<p class="myCardd downPriem"><strong>Длительность:</strong> </p>							
							</div>
							<div class="col-md-10 text-center">
							<input type="text"  name="text" placeholder="<?=$Diag[0]['CourseDuration']; ?>">						
							</div>
						</div>	
